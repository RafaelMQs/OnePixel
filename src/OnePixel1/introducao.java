...
public class introducao extends JFrame {
	...
	private String[] TextoDoGuri = { "<html>Guri: Quem � voc�? O que Aconteceu? Onde Estou?",
			"<html>Guri: Por que s� eu estou aqui? Cad� meus pais?",
			// CASO ELE ESCOLHA MORRER
			"<html>Guri: Eu escolho morrer!",
			"<html>Guri: Eu sou um p�ssimo filho. Eu sou apenas um ego�sta. Um ego�sta NAO pode salvar o mundo..",
			"<html>Guri: !?",
			"<html>Guri: He he... Pensando bem... Posso mudar de op��o?",
	...
	private String[] TextoDoZero = {
			"<html>Zero: Eu me chamo Zero, sou o servidor do mundo virtual. Voc� est� na nuvem da rede de internet. Por algum motivo houve um bug no sistema e todos os pixels se apagaram.",
			"<html>Zero: Voc� � a �nica pessoa que encontrei nessa escurid�o. Aproveitando o momento, te escolho para uma miss�o extremamente importante. Voc� ter� que encontrar os 3 pixels e reacend�-los com esta tocha m�gica, estou com um pouco de pregui�a pra isso.",
			"<html>Zero: Agora sobre os seus pais... Eu NAO irei dizer nada, ent�o tire suas pr�prias conclus�es nessa jornada. Agora voc� tem duas op��es: <br> <br> <center>1� MORRER AGORA ou 2� IR PARA A MISS�O",
			// CASO ELE ECOLHA MORRER
			"<html>Zero: ???????????????????????????????",
			"<html>Zero: Na verdade um ego�sta pode sim salvar o mundo, basta deixar de ser um ego�sta.",
			"<html>Zero: Use essa miss�o de pretexto para tornar-se altru�sta. Voc� NAO pode consertar os seus erros, mas pode nunca mais comet�-los.",
			"<html>Zero: Como voc� foi o �nico que encontrei irei permitir voc� trocar de op��o. Por favor, d� seu melhor.",
			"<html>Zero: Basta seguir o seu cora��o"}; // Texto do Guri
		...
		public void run() {
			try {
				...
				for (int z = 0; z < TextoDoGuri[0].length(); z++) {
					if (!pularDialog) {
						// FUN��O QUE GUARDA LETRA POR LETRA EM UMA VARIAVEL E VAI MOSTRANDO
						TextEffect(TextoDoGuri[0], dialogoDoGuri, z, 60);
					} else {
						// CASO ELE APERTE ESPA�O ELE ACABA O FOR E COMPLETA A FRASE
						dialogoDoGuri.setVisible(true);
				...
				Op2NoSelected.setVisible(false);
				Op2Selected.setVisible(false);
				// DESATIVA A PERMISS�O DE FAZER ESCOLHAS
				fazerEscolha = false;
